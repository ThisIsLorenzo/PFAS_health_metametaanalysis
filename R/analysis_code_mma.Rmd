...
ps_details <- read.csv(here("data/ps_details.csv"))

amstar2 <- amstar2 %>%
  # Remove all columns ending with "Comment"
  select(-ends_with("Comment")) %>%  # Select columns
  # Rename by extracting question numbers
  rename_with(
    ~ case_when(
      . == "ma_id" ~ "ma_id",
      . == "ma_doi" ~ "ma_doi",
      str_detect(., "^Q(\\d+)\\.") ~ paste0("Q", str_extract(., "(?<=^Q)\\d+")),  # Detect pattern matches in strings
      TRUE ~ .
    ),
    .cols = everything()
  ) %>% 
  mutate(across(starts_with("Q"), ~ as.numeric(str_extract(., "^\\d+\\.?\\d*"))))  # Create/modify columns
...
dat_qual <- ma_e_details %>%
  ...
  filter(chemical_class == "PFAS") %>%  # Filter rows by conditions
  ...
  left_join(ma_details, by = "ma_id", suffix = c("", ".remove")) %>%  # Join tables keeping all rows from the left table
...
ps_details <- ps_details %>% 
  ...
  select("ma_e_id", "ma_id", "primary_doi", "chemical")
```
...
source(here("Rdata/functions", "conversion.R"))
...
```{r , include=FALSE}
dat_quant <- dat_quant %>%
  group_by(chemical_id, health_outcome_group) %>%  # Define grouping structure for summaries
  mutate(
...
# custom function
source(here("Rdata/functions", "build_vcv_from_overlap.R"))

# Build VCV using observed overlap of primary studies (global across all estimates)
...
VCV_test <- vcv_obj$V
dat_quant_test <- vcv_obj$dat_vcv
```


```{r, eval=FALSE}
mod_test <- rma.mv(yi = logOR,  # Fit a multilevel/multivariate meta-analysis model
              V = VCV_test, 
              random = list(~1|ma_id, 
                            ~1|ma_e_id), 
...
save(mod_test, file = here("Rdata", "models/mod_test.RData"))
```


```{r, echo=FALSE}
load(here("Rdata/models", "mod_test.RData"))
# Display model results
summary(mod_test)
```

```{r , echo=FALSE, warning=FALSE}
orc_plot_test <- orchard_plot(  # Create an orchard plot for meta-analytic models
  object = mod_test,
  mod = "ma_es_measure",
  group = "ma_id",
  xlab = "log Odds Ratio",
...
```{r, eval=FALSE}
ggsave(here("Rdata/figures/supplements","Supplementary Figure 1.png"),
       plot = orc_plot_test,
       width = 7,
       height = 6)
```

The moderator analysis revealed a statistically significant impact of the effect size metric on the overall estimates. 
...
ggplot(pub_year_data, aes(x = pub_year)) +  # Initialize a ggplot object
...
chemical_count <- dat_quant %>%
  ...
  summarise(  # Summarise data (often by groups)
  ...
  arrange(desc(`Number of meta-analyses`))  # Sort rows
...
```{r , include = FALSE}
# custom function
#source(here("Rdata/functions", "custom_logOR.R"))
source(here("Rdata/functions", "custom_logOR_new.R"))
...
est_dat <- est_dat %>% 
  group_modify(~ custom_meta_aggregate_logOR_new(.x, ps_details = ps_details)) %>% ungroup()

saveRDS(
  est_dat,
  file = here("Rdata/data", "est_dat.rds")   # saves est_dat as a single R object
)
```

```{r, echo=FALSE, out.height=600, out.width=1000, fig.width=11, fig.height=10}
est_dat <- readRDS(here("Rdata/data", "est_dat.rds"))  # restores est_dat
...
```{r, eval=FALSE, include=FALSE}
ggsave(here("Rdata/figures/supplements","Supplementary Figure 2.png"),
       width = 8,
       height = 8)
```
...
ggsave(here("Rdata/figures/supplements","Supplementary Figure 3.png"),
       width = 8,
       height = 8)
```
...
saveRDS(
  est_dat2,
  file = here("Rdata/data", "est_dat2.rds")
)
```

```{r, include=FALSE}
est_dat2 <- readRDS(here("Rdata/data", "est_dat2.rds"))  # restores est_dat
...
saveRDS(
  est_dat3,
  file = here("Rdata/data", "est_dat3.rds")
)
```

```{r ,include=FALSE}
est_dat3 <- readRDS(here("Rdata/data", "est_dat3.rds"))  
...
saveRDS(
  est_dat4,
  file = here("Rdata/data", "est_dat4.rds")   # saves est_dat as a single R object
)
```

```{r, include=FALSE}
est_dat4 <- readRDS(here("Rdata/data", "est_dat4.rds")) 
...
saveRDS(
  est_dat5,
  file = here("Rdata/data", "est_dat5.rds")
)
```

```{r ,include=FALSE}
est_dat5 <- readRDS(here("Rdata/data", "est_dat5.rds"))  # restores est_dat
...
saveRDS(
  est_dat6,
  file = here("Rdata/data", "est_dat6.rds")   # saves est_dat as a single R object
)
```

```{r, include=FALSE}
est_dat6 <- readRDS(here("Rdata/data", "est_dat6.rds"))  # restores est_dat
...
saveRDS(
  est_dat7,
  file = here("Rdata/data", "est_dat7.rds")   # saves est_dat as a single R object
)
```

```{r, include=FALSE}
est_dat7 <- readRDS(here("Rdata/data", "est_dat7.rds"))  # restores est_dat
...
Fig_s4 <- plot_grid(Box2_map1, Box2_map6, Box2_map2, Box2_map3, Box2_map4, Box2_map5, 
...
```{r,  eval=FALSE, include=FALSE}
ggsave(here("Rdata/figures/supplements","Supplementary Figure 4.png"),
       plot = Fig_s4,
       width = 10,
       height = 14)
```
...
```{r, results=FALSE}
# 1) Make sure the estimate id and variances are aligned
dat_vcv <- dat_quant %>%
  mutate(
    estimate_id = ma_e_id,
    vi = se^2
...
# 2) Build sparse incidence matrix A: (estimate_id x primary_doi)
ps_long <- ps_details %>%
  transmute(
    estimate_id = ma_e_id,
...
# Keep only DOIs for estimates that are actually in the model dataset
ps_long <- ps_long %>%
  semi_join(dat_vcv %>% select(estimate_id), by = "estimate_id")

# Integer indices for sparseMatrix()
...
A <- sparseMatrix(
  ...
  dimnames = list(dat_vcv$estimate_id, doi_levels)
)

# 3) Shared primary-study counts (sparse)
shared <- tcrossprod(A)  # n_ij = number of shared DOIs

# 4) n_i (number of DOIs per estimate)
n_i <- Matrix::rowSums(A)

# 5) Build rho matrix ONLY where shared > 0 (keep sparse)
# cosine overlap: rho_ij = n_ij / sqrt(n_i * n_j)
# We'll compute elementwise on the non-zero entries of `shared`.
shared_t <- summary(shared)  # triplet form: i, j, x
...
rho <- sparseMatrix(
  ...
  x = rho_x,
  dims = dim(shared),
  dimnames = dimnames(shared)
)

# Ensure diagonal rho = 1 for estimates with >=1 DOI, else 0
diag(rho) <- ifelse(n_i > 0, 1, 0)

# 6) Convert rho to VCV: V_ij = rho_ij * sqrt(vi_i * vi_j)
vi <- dat_vcv$vi
shared_rho_t <- summary(rho)

vcv_x <- shared_rho_t$x * sqrt(vi[shared_rho_t$i] * vi[shared_rho_t$j])
...
# Guarantee exact diagonal equals vi (metafor likes this)
diag(VCV) <- vi

# (Optional but often good) force symmetry
VCV <- forceSymmetric(VCV, uplo = "L")
```
...
save(mod_overall, file = here("Rdata/models", "mod_overall.RData"))
```

```{r, include=FALSE}
load(here("Rdata/models", "mod_overall.RData"))
...
ps_long_lnH <- ps_details %>%
  transmute(
    estimate_id = ma_e_id,
    primary_doi = primary_doi
  ) %>%
  distinct() %>%
  semi_join(dat_lnH_vcv %>% select(estimate_id), by = "estimate_id")

i_idx <- match(ps_long_lnH$estimate_id, dat_lnH_vcv$estimate_id)
doi_levels <- sort(unique(ps_long_lnH$primary_doi))
j_idx <- match(ps_long_lnH$primary_doi, doi_levels)

A <- sparseMatrix(
  i = i_idx,
  j = j_idx,
  x = 1,
  dims = c(nrow(dat_lnH_vcv), length(doi_levels)),
  dimnames = list(dat_lnH_vcv$estimate_id, doi_levels)
)
...
diag(VCV_lnH) <- vi
VCV_lnH <- forceSymmetric(VCV_lnH, uplo = "L")
```
...
save(mod_overall_lnH, file = here("Rdata/models", "mod_overall_lnH.RData"))
```


```{r, include=FALSE}
load(here("Rdata/models", "mod_overall_lnH.RData"))
...
```{r, include=FALSE, eval=FALSE}
ggsave(here("Rdata/figures","Figure 2.png"),
       plot = fig2,
...
write_csv(
  table1,
  file = here("Rdata/tables", "Table 1.csv")
)
```
...
results_list <- map_dfr(chemicals, function(chem) {
  ...
  if (n_distinct(dat_sub$ma_e_id) > 1) {  # Require at least 2 meta-analytic estimates
    tryCatch({
      # Calculate VCV matrix using shared primary studies
vcv_obj <- build_vcv_from_overlap(
  dat_sub = dat_sub,
  ps_details = ps_details,
  id_col = "ma_e_id",
...
VCV_sub <- vcv_obj$V
dat_sub2 <- vcv_obj$dat_vcv  # reordered to match VCV
...
```{r, eval=FALSE, include=FALSE}
ggsave(here("Rdata/figures","Figure 3.png"),
       plot = fig3,
...
```{r , include = FALSE}
# custom function
#source(here("Rdata/functions", "custom_lnH.R"))
source(here("Rdata/functions", "custom_lnH_new.R"))
# get estimate for each cell
est_dat_lnH <- dat_lnH %>% group_by(chemical_id, health_outcome_group) %>%
  ...
  group_modify(~ custom_meta_aggregate_lnH_new(.x, ps_details = ps_details)) %>% ungroup() %>%
  mutate(
    # 1) Compute H2 and its CIs
    H2        = exp(2 * lnH),
...
save_as_docx(
  ft, 
  path = here("Rdata/tables", "Supplementary Table 8.docx")
)
```
...
amstar2_long <- amstar2 %>%
  ...
  pivot_longer(cols = starts_with("Q"),  # Reshape data from wide to long format
               names_to = "question", 
               values_to = "score") %>%
...
print(doc, target = here("Rdata/tables", "Supplementary Table 10.docx"))
...
```{r, eval=FALSE, include=FALSE}
# Save the plot
ggsave(here("Rdata/figures/supplements","Supplementary Figure 6.png"), 
       plot = forest_plot, 
       width = 8, height = 6, 
       dpi = 300)
```
...
run_loo_sensitivity <- function(data, excluded_ma_id, ps_details) {
  ...
  # Recalculate covariance matrix for filtered data
  vcv_obj <- build_vcv_from_overlap(
  dat_sub    = filtered_data,
  ps_details = ps_details,
  id_col     = "ma_e_id",
  vi_col     = "se"
)

VCV_filtered <- vcv_obj$V
filtered_data2 <- vcv_obj$dat_vcv
  ...
  tryCatch({
    ...
    tibble(
      excluded_study = excluded_ma_id,
      k = nrow(filtered_data2),
n = n_distinct(filtered_data2$ma_id),
...
loo_results <- map_dfr(
  unique_studies,
  ~ run_loo_sensitivity(dat_quant, .x, ps_details = ps_details)
)
...
ggsave(here("Rdata/figures/supplements","Supplementary Figure 5.png"), 
       plot = combined_plot, 
       width = 12, height = 10, 
       dpi = 300)
...
print(enhanced_doc, target = here("Rdata/tables", "Supplementary Table 9.docx"))
```